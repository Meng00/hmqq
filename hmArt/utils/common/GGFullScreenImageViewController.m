...
- (void) loadView {
    ...
    CGFloat x = (rect.size.width - _imageSize.width) / 2;
    CGFloat y = (rect.size.height - _imageSize.height) / 2;
    CGRect imageRect = CGRectMake(x, y, _imageSize.width, _imageSize.height);
    self.imageView = [[UIImageView alloc] initWithFrame:imageRect];
    //self.imageView = [[UIImageView alloc] initWithFrame:_imageRect];
...
- (void) viewWillAppear:(BOOL)animated {
    ...
    if (UIInterfaceOrientationIsPortrait(orientation)) {
        ...
        self.imageView.layer.bounds = startFrame;
    } else {
...
- (void) viewDidAppear:(BOOL)animated {
    ...
    NSLog(@"startFrame:%@", NSStringFromCGRect(self.imageView.frame));
    CGRect rect = [[UIScreen mainScreen] bounds];
    CGRect startFrame = self.imageView.frame;
    endFrame.origin.x = (rect.size.width - startFrame.size.width) / 2;
    endFrame.origin.y = (rect.size.height - startFrame.size.height) / 2;
