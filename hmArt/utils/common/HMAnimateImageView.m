...
@implementation HMAnimateImageView
...
- (void)initMyself
{
    ...
    self.image = [UIImage imageNamed:@"animatedImageDef.png"];
    _switching = NO;
    _interval = 5;
...
{
    [_adItems addObject:imgUrl];
    UIImage *image = [UIImage imageNamed:@"animatedImageDef.png"];
    [_adImages addObject:image];
    if (_adItems.count == 1) {
        [self enableInteraction];
    }
   
    NSString *imageUrl = [NSString stringWithFormat:@"%@%@", HM_SYS_IMGSRV_PREFIX, imgUrl];
    unsigned int requestId = [[LCNetworkInterface sharedInstance] asynExternalRequest:imageUrl needSSL:NO cached:YES target:self action:@selector(serviceComplete:withResult:)];
...
{
    if (_delegate && [_delegate respondsToSelector:@selector(animatedImageView:tapAtIndex:)]) {
        [_delegate animatedImageView:self tapAtIndex:_current];
    }
